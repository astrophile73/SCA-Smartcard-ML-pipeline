...
def extract_features(dataset_path, poi_indices, output_dir="Processed", file_pattern="traces_data_*.npz", skip_poi_search=False, separate_sboxes=False, card_type="universal", include_secrets=True, trace_type="all", opt_dir=None):
    """
    Extract POIs from all traces with robust alignment and Dual-Pass Global POI selection.
    """
    ds = TraceDataset(dataset_path, file_pattern=file_pattern, card_type=card_type, trace_type=trace_type)
...
def perform_feature_extraction(
    ...
    if include_secrets and os.path.exists(meta_path):
        cols = pd.read_csv(meta_path, nrows=1).columns.tolist()
